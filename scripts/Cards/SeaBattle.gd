class_name SeaBattle extends CardLogicBase

var _attackable_units:Array[int]:
	get: return GameManager.game_state.attackable_units_for_faction(faction).filter(func(_unit_id:int): return UnitState.for_id(_unit_id).country_state.is_sea)	
...
func _request_country():	
	SelectSingleUnitHandler.new(_attackable_units).handle(
		func(_selected_unit_id):
			...
			await ChangeEventHandler.execute_change_event(_battle_unit_change_event).change_event_finished
			card_play_finished()
	)
