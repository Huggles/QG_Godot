...
public partial class DebugUtilities : Node
{
	...
	/// <summary>
	/// True when console logging must stay off stdout: a CLI session uses stdout as a line protocol,
	/// and these helpers write BBCode colour tags through GD.PrintRich, which would corrupt it.
	/// `verbose=true` opts back in for debugging the CLI itself.
	/// PrintPeerErrorRaw is deliberately exempt — it writes to stderr.
	/// </summary>
	private static bool Muted => GameContext.IsCli && !CliArgs.Verbose;
