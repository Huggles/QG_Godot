...
public partial class StatusForwardBases : StatusCardLogic, IVPModifier
{
    public virtual VPEntry AddVictoryPoints()
    {
        ...
        return new VPEntry(score, $"{FactionState.ForEnum(Faction).FactionData.FactionAdjactiveLabel} army in {CountryState.ForEnum(countries[0]).Label}, {CountryState.ForEnum(countries[1]).Label} or {CountryState.ForEnum(countries[2]).Label}");
    }
