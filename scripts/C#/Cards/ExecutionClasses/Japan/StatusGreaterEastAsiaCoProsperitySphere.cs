...
public partial class StatusGreaterEastAsiaCoProsperitySphere : StatusCardLogic, IVPModifier
{
    public virtual VPEntry AddVictoryPoints()
    {
        ...
        return new VPEntry(score, $"{FactionState.ForEnum(Faction).FactionData.FactionAdjactiveLabel} armies in {CountryState.ForEnum(countries[0]).Label}, {CountryState.ForEnum(countries[1]).Label} and {CountryState.ForEnum(countries[2]).Label}");
    }
