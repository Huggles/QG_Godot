...
#
# Usage: .\quick_launch_two.ps1 [-Clients 2]

param(
    [int]$Clients = 1
)

# MultiplayerLobby hosts with ENetMultiplayerPeer.CreateServer(port, 2): two client slots.
if ($Clients -lt 1 -or $Clients -gt 2) {
    Write-Host "Clients must be between 1 and 2 (the host only opens 2 client slots)." -ForegroundColor Red
    exit 1
}
